...
(defun lsp-rust-diagnostics-show-primary (err)
  ...
  (let* ((string (flycheck-error-message err))
         (strings (--map-first t (propertize it 'face '(:foreground "red")) (split-string string "\n\n" t)))
         label)
    ...
    ;; (setq strings (--map-first t (propertize it 'face 'error) strings))
    (setq strings (--map-first t (propertize it 'face '(:foreground "red" :weight 'ultra-bold)) strings))

    (setq strings (if (>= (length strings) 3)
                      (string-join (-remove-at 1 strings) "\n\n")
;;;                    (string-join (-remove-at 1 strings) "\n\n")
                    (car strings)))
    (lsp-ui-doc--display 'a strings)
    ;; (lsp-ui-doc--display 'a (propertize strings 'face '(:foreground "red")))
    (push (list :line (flycheck-error-line err) :label label) lsp-rust-diagnostics-labels)
;;;    (push (cons (flycheck-error-line err) label) lsp-rust-diagnostics-labels)
    ))
...
;; (defun lsp-rust-diagnostics--show-label (line label max-len)
;;   "LINE LABEL MAX-LEN."
;;   (let* ((pos (save-excursion
;;                 (goto-char (point-min))
;;                 (line-end-position line)))
...
;;         (when (> (length label) 0)
;;           (lsp-rust-diagnostics--show-label line label max-len))))))
...
;;                       (string-join (-remove-at 0 strings))
;;                     (car strings))))
;;       (lsp-rust-diagnostics--range-to-points range)
;;       (push (cons line label) lsp-rust-diagnostics-labels)
;;       )
;;     ))
